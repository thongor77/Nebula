...
Nebula's second official theme — light variant. Frosted glass, Fluent
...
`glass-dark` (see [`../glass-dark/`](../glass-dark/)) shares every
...
- `assets/wallpapers/glass-light.png` (3840×2160) +
  `glass-light-compressed.jpg` — an original, low-saturation light
...
- `assets/icons/{shutdown,restart,suspend,hibernate,reveal-password}.png`
  ...
  shared identically with `glass-dark` (the gray tone reads on both
  backgrounds). **Not currently wired into the real UI**: neither
  `NebulaPowerButtons` nor `NebulaPasswordField` expose an icon property
  yet — a real, known Core limitation, documented rather than fixed
  (see `Glass-Theme-Report.md`, Constat #1). The icons are ready for
  when that Core evolution happens.
